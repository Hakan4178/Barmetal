...
static void count_snapshot_entries(struct mm_struct *mm, u64 *vma_count, u64 *map_count,
				   u64 *data_size)
{
				...
				if (pg && page_count(pg) > 0 && !PageReserved(pg)) {
					...
					if (vsrc && (raw_off + chunk_size) <= data_size_est) {
						memcpy(raw_buf + raw_off,
						       (u8 *)vsrc + offset_in_page, chunk_size);
...
static int pl_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%-8s %-20s\n", "PID", "NAME");
	rcu_read_lock();
	for_each_process(task) { seq_printf(m, "%-8d %-20s\n", task_pid_nr(task), task->comm); }
	rcu_read_unlock();

	return 0;
}

static int pl_open(struct inode *i, struct file *f) { return single_open(f, pl_show, NULL); }
...
static int st_open(struct inode *i, struct file *f) { return single_open(f, st_show, NULL); }
